...
var configPlugins = [
	...
	new CopyWebpackPlugin([
		{
			from : dirConfig.srcDir+"/static/css",//打包的静态资源目录地址
			to : path.resolve(dirConfig.buildDir ,"./css/"), //打包到build下面的css
			flatten : false
		},
		{
			from : dirConfig.srcDir+"/static/js",//打包的静态资源目录地址
			to : path.resolve(dirConfig.buildDir ,"./js/"), //打包到build下面的js
			flatten : false,
		},
		{
			from : dirConfig.srcDir+"/static/images",//打包的静态资源目录地址
			to : path.resolve(dirConfig.buildDir ,"./images/"), //打包到build下面的images
			flatten : true
		},
		{
			from : dirConfig.srcDir+"/static/commons",//打包的静态资源目录地址
			to : path.resolve(dirConfig.buildDir ,"./commons/"), //打包到build下面的commons
			flatten : true
		},
	]),
	/* 引用 manifest.json 文件 */
...
pageList.forEach((page) => {
	var htmlPlugin = new HtmlWebpackPlugin({
		...
		inject : 'body',
		minify: true,
		favicon : path.resolve(dirConfig.rootDir ,"./favicon.ico"),
	});
